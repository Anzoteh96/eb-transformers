...
def main():
    ...
    parser.add_argument('--theta_max', type=float, default=50, help='Max theta value')
    parser.add_argument('--m', type=int, default=80, help='Number of examples in batch')
    parser.add_argument('--seqlen', type=int, default=512, help='Sequence length')
    parser.add_argument('--dinput', type=int, default=1, help='Input dimension (default: 1)')
    ...
    # For compatibility with gen_priors.py and GoofyPrior
    args.batch = args.m

    # Generate batch and get prior
    (_, labels), prior = gen_batch_from_seed(args, return_prior=True)
    # Always call gen_thetas to get thetas for GoofyPrior
    if hasattr(prior, 'gen_thetas'):
        ...
        print("Prior does not have a 'gen_thetas' method. Inspect the prior object:", type(prior))

...
def main():
    ...
    print("RUN ARGS:", args)
        ...
        # --- finalize m: prefer CLI --m; else fall back to the trained model's m ---
    trained_m = getattr(model_args, 'm', None) if model_args is not None else None
    if args.m is None and trained_m is not None:
        args.m = trained_m
    elif trained_m is not None and args.m is not None and args.m != trained_m:
        print(f"WARNING: run m={args.m} != model-trained m={trained_m} (OOD eval).", flush=True)
        
    # --- now it's safe to build a prior that depends on m ---
    if args.same_prior:
        set_seed(args.prior_seed)
        args.prior = eb_train.gen_random_prior(args)
